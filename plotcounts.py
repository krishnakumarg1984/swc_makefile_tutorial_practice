...
def plot_word_counts(counts, limit=10):
    """
    Given a list of (word, count, percent) tuples, plot counts as a histogram.
    Only the first limit tuples are plotted.
...
def typeset_labels(labels=None, gap=5):
    """
    Given a list of labels, create a new list of labels such that each label is
    right-padded by spaces so that every label has the same width, then is
    further right padded by ' ' * gap.
    """
...
def get_ascii_bars(values, truncate=True, maxlen=10, symbol="#"):
    ...
    strings contains N symbols where
    N = ()(value / minimum) / (maximum - minimum)) * (maxlen / len(symbol)).
    """
    maximum = max(values)
    if truncate:
